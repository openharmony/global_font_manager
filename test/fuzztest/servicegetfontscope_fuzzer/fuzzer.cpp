...
void ServiceGetFontScopeFuzz(const std::string &srcPath)
{
    std::string processName = "ServiceGetFontScopeFuzz";
    ...
    int32_t result;
    (void)service->GetFontScope(srcPath, result);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* input, size_t size)
{
    ...
    ServiceGetFontScopeFuzz(srcPath);
    return 0;
}
} // namespace FontManager
} // namespace Global
