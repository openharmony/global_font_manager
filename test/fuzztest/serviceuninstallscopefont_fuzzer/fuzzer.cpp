...
void ServiceUninstallScopeFontFuzz(const std::string &srcPath)
{
    std::string processName = "ServiceUninstallScopeFontFuzz";
    ...
    int32_t result;
    (void)service->UninstallScopeFont(srcPath, result);
    PermissionCommon::ResetTokenAndUid();
}

extern "C" int LLVMFuzzerTestOneInput(const uint8_t* input, size_t size)
{
    std::string srcPath = NewString(input, size);
    ServiceUninstallScopeFontFuzz(srcPath);
    return 0;
}
} // namespace FontManager
} // namespace Global
