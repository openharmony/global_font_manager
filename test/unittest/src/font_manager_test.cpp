...
bool FontManagerTest::CopyFile(const string &srcPath, const string &desPath)
{
...
HWTEST_F(FontManagerTest, FontManagerFuncTest001, TestSize.Level1)
{
    ...
    EXPECT_EQ(fd >= 0, true);
    int ret = this->manager_.InstallFont(fd, TEST_USERID);
    EXPECT_EQ(ret, ERR_OK);
    if (fd >= 0) {
        close(fd);
    }
    fd = open(FONT_PATH.c_str(), O_RDONLY);
    ret = this->manager_.InstallFont(fd, TEST_USERID);
...
HWTEST_F(FontManagerTest, FontManagerFuncTest002, TestSize.Level1)
{
    ...
    ret = this->manager_.UninstallFont(FONT_FULL_NAME, TEST_USERID);
    EXPECT_EQ(ret, ERR_OK);
    ret = this->manager_.UninstallFont(FONT_FULL_NAME, TEST_USERID);
    EXPECT_EQ(ret, ERR_UNINSTALL_FILE_NOT_EXISTS);
    ret = this->manager_.UninstallFont("", TEST_USERID);
    EXPECT_EQ(ret, ERR_UNINSTALL_FILE_NOT_EXISTS);
}
...
{
    int ret = this->manager_.CheckFontConfigPath(INSTALL_PATH_TEST);
    EXPECT_EQ(ret, true);
    FileUtils::RemoveFile(FONT_CONFIG_FILE_TEST);
    ret = this->manager_.CheckFontConfigPath(INSTALL_PATH_TEST);
    EXPECT_EQ(ret, true);
    ret = this->manager_.CheckFontConfigPath(INSTALL_PATH_TEST);
    EXPECT_EQ(ret, true);
}
...
HWTEST_F(FontManagerTest, FontManagerFuncTest009, TestSize.Level1)
{
    ...
    ret = this->manager_.UninstallFont(TTC_FONT_FULL_NAME[0], TEST_USERID);
    EXPECT_EQ(ret, ERR_OK);
    ret = this->manager_.UninstallFont(TTC_FONT_FULL_NAME[0], TEST_USERID);
    EXPECT_EQ(ret, ERR_UNINSTALL_FILE_NOT_EXISTS);
    ret = this->manager_.UninstallFont("", TEST_USERID);
...
HWTEST_F(FontManagerTest, FontManagerFuncTest014, TestSize.Level1)
{
    ...
    bool res = this->manager_.InitAllUserDir({TEST_USERID});
    ASSERT_TRUE(res);
    res = this->manager_.InitDataMigrationTempDir();
    ASSERT_TRUE(res);
    res = this->manager_.InitDataMigrationTempDir();
    ASSERT_TRUE(res);
    res = this->manager_.CopyFileForDataMigration(fontPath, TEST_USERID);
    EXPECT_EQ(res, true);
    res = this->manager_.CopyFileForDataMigration(fontPath, TEST_USERID);
    EXPECT_EQ(res, true);
...
{
    std::string processName = "FontManagerFuncTest015";
    ...
    this->manager_.DataMigration(cb);
    this->manager_.EventDataHeartBeatCallback(cb);
...
{
    std::string processName = "FontManagerFuncTest017";
    SetFontManagerPermission(processName);
    sptr<IDataMigrationCallback> cb = new (std::nothrow) TestCallback();
    if (cb == nullptr) {
        return;
